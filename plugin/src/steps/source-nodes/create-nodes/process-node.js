...
const replaceNodeHtmlLinks = ({
  ...
  if (!helpers.pathPrefix || !pluginOptions.html.prefixRelativeUrls)
    return nodeString
  ...
  const relativeLinkRegex = new RegExp(
    `href=[\\\\]{0,2}["']((?!${helpers.pathPrefix}|/wp-content|/wp-admin|/wp-includes|//)/[^'"]+)[\\\\]{0,2}["']`,
    `gim`
  )
      ...
      if (path) {
        try {
          ...
          const prefixedPath = `${
            !normalizedPath
              .toLowerCase()
              .startsWith(helpers.pathPrefix.toLowerCase())
              ? helpers.pathPrefix
              : ``
          }${normalizedPath}`
          // group the surrounding to use in replace $1 and $3
          const relativeMatchRegex = new RegExp(
            `(href=[\\\\]{0,2}["'])((?!${helpers.pathPrefix}|/wp-content|/wp-admin|/wp-includes|//)${normalizedPath})([\\\\]{0,2}["'])`,
            `g`
          )
