...
Now we have ability to properly cache data which makes incremental builds, fast/cacheable builds, and Preview work. Any WPGraphQL plugin now becomes a cacheable Gatsby plugin which means the Gatsby and the community can build a rich plugin ecosystem that just works.
...
Well the great thing about it is you can keep using WordPress as the fantastic CMS it is, but use React and modern tooling for your front end. 

- Increased security and performance due to the static rendering of Gatsby
- A componentized architecture for your front-end code with React which leads to better maintainability
...
Additionally, this plugin is a great reason to use Gatsby with WordPress. Many of the complex and difficult aspects of going decoupled with WordPress have been abstracted for you behind the scenes, so you can work on building awesome apps and sites instead of reinventing the wheel to get decoupled WordPress working again in each project.

Check out this page on [convincing developers why Gatsby is awesome](https://www.gatsbyjs.org/docs/winning-over-developers/#specific-benefits). It's meant for developers convincing their coworkers about Gatsby but actually does a great job of describing directly to you the main reasons why using Gatsby is a great idea :)
